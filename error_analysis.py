...
def get_value_dict(content, tgt_form):
	t = type(content)
	forbidden = 96752 # TODO deal with types, argh # TODO make sure unused
	dummy_content = t(*([forbidden] * len(content._fields)))
	form_primitives = list(set(tgt_form.function(dummy_content).split(str(forbidden))))
	form_primitives = list(filter(lambda x: x, form_primitives))
	value_dict = {
